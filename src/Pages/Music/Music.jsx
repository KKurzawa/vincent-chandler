...
const Music = () => {
    return (
        <main className='music-main flex flex-col justify-center items-center gap-10 md:gap-20'>
            ...
            <article className='flex flex-col lg:flex-row justify-center items-center gap-10 lg:gap-40'>
                ...
                <section className='audio-player-container flex flex-col justify-center items-center gap-1 md:gap-5'>
                    <h2 className='stream-music text-3xl'>Stream Music</h2>
                    <AudioPlayer className='audio-player' />
                </section>
            </article>
            <article className='purchase-music-container flex flex-col justify-center items-center gap-1 md:gap-5'>
                <h2 className='purchase-music-title text-3xl'>Purchase Music</h2>
                <section className='album-container-container flex gap-5 md:gap-10 lg:gap-40'>
                    <section className='left-album-container flex flex-col gap-5 md:gap-10 lg:gap-20'>
                        {Albums.map((album, index) => index % 2 === 0 && (
                            <button onClick={() => setTimeout(() => window.open(album.link, '_blank'), 500)}>
                                <img src={album.cover} className='album-cover w-[170px] md:w-[350px] lg:w-[500px] h-auto rounded-md' />
                            </button>
                        ))}
                    </section>
                    <section className='left-album-container flex flex-col gap-5 md:gap-10 lg:gap-20'>
