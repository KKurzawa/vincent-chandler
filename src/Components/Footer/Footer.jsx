...
const Footer = () => {
    ...
    return (
        <footer className='footer-main flex flex-col justify-center items-center gap-5'>
            <section className='media-icons flex gap-10 mt-10'>
                {socialMediaLinks.map((item) => (
                    <button className='media-icon text-7xl' onClick={() => setTimeout(() => window.open(item.link, '_blank'), 500)}>{item.icon}</button>
                ))}
            </section>
            ...
            <section className='copyright-devLink flex flex-col justify-center items-center mb-10'>
